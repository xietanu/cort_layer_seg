            ...
            try:
                ...
                for idx, section in zip(idxs, features):
                    ...
                    try:
                        ...
                        if (max_pnt[0] - min_pnt[0]) < 12:
                            min_pnt[0] -= (12 - max_pnt[0] + min_pnt[0]) / 2
                            max_pnt[0] = min_pnt[0] + 12

                        if (max_pnt[2] - min_pnt[2]) < 12:
                            min_pnt[2] -= (12 - max_pnt[2] + min_pnt[2]) / 2
                            max_pnt[2] = min_pnt[2] + 12
                        ...
                        bigbrain_chunk = bigbrain_template.fetch(
                            voi=bbox, resolution_mm=0.02
                        )
                        ...
                        y_excess = max(img.shape[0] - 512, 0)
                        x_excess = max(img.shape[1] - 512, 0)
                        img = img[
                        ...
                        store_patch(img, position, area, hemisphere, idx)
...
def store_patch(img, pos, area_name, hemisphere, section_n):
    ...
    os.makedirs(f"data/random_imgs/{region_name}", exist_ok=True)
    cv2.imwrite(f"data/random_imgs/{region_name}/img.png", img)
    np.save(f"data/random_imgs/{region_name}/coords.npy", coords)
    np.save(f"data/random_imgs/{region_name}/probs.npy", prob)
    info = {
        "area": area_name,
        "hemisphere": hemisphere,
        "section_n": section_n,
    }
    json.dump(info, open(f"data/random_imgs/{region_name}/info.json", "w"))
