...
class ConditionalNNetModel(nnet.protocols.ModelProtocol):
    """A U-Net model."""
    ...
    def __init__(
        ...
        self.network = nnet.modules.ConditionalNNet(
    ...
    def _calc_loss_acc(self, inputs, targets):
        image, condition = nnet.util.send_to_device(inputs, self.device)
        targets = nnet.util.send_to_device(targets, self.device)

        if isinstance(targets, tuple):
            targets = targets[0]

        outputs = self.network(image.float(), condition.float())
