...
class UnetPlusPlusBackbone(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        for i, (process_block, compress_block) in enumerate(
            zip(self.process_conv_blocks, self.compress_conv_blocks)
        ):
            x = process_block(x)
            features.append(x)
            x = compress_block(x)
...
class UnetPlusPlusUpsampler(torch.nn.Module):
    ...
    def forward(
        self, residuals: list[torch.Tensor], to_upsample: torch.Tensor
    ) -> torch.Tensor:
